...
def train_epoch(model, train_loader, optimizer, device, kl_weight, score_weight):
    ...
    for color_data, text_data, scores in train_loader:
...
def calculate_metrics(model, train_loader, device):
    ...
    # 主题一致性
    def topic_coherence(matrix):
        coherence_scores = []
        for topic_idx in range(matrix.shape[0]):
            top_indices = np.argsort(matrix[topic_idx])[-10:]
            top_weights = matrix[topic_idx][top_indices]
            coherence = np.mean(top_weights)
            coherence_scores.append(coherence)
        return np.mean(coherence_scores)
    
    metrics['Topic_Coherence_Mod1'] = topic_coherence(topic_color_matrix)
    metrics['Topic_Coherence_Mod2'] = topic_coherence(topic_word_matrix)
    
    # 主题稀疏性
    def topic_sparsity(matrix, threshold=0.01):
        return 1.0 - (np.count_nonzero(matrix > threshold) / matrix.size)
    
    metrics['Topic_Sparsity_Mod1'] = topic_sparsity(topic_color_matrix)
    metrics['Topic_Sparsity_Mod2'] = topic_sparsity(topic_word_matrix)
    
    # 主题特异性
    def topic_specificity(matrix):
        entropy = -np.sum(matrix * np.log(matrix + 1e-10), axis=1)
        specificity = 1.0 / (entropy + 1e-10)
        return np.mean(specificity)
    
    metrics['Topic_Specificity_Mod1'] = topic_specificity(topic_color_matrix)
    metrics['Topic_Specificity_Mod2'] = topic_specificity(topic_word_matrix)
    
    # 跨模态对齐
    correlation = np.corrcoef(topic_color_matrix.flatten(), topic_word_matrix.flatten())[0, 1]
    metrics['Cross_Modal_Alignment'] = correlation if not np.isnan(correlation) else 0.0
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        # 每10轮评估一次
        if (epoch + 1) % 10 == 0:
            metrics = calculate_metrics(model, train_loader, device)
    ...
    torch.save(model.alpha.detach().cpu(), os.path.join(matrices_dir, 'final_alpha.pt'))
    torch.save(model.rho_color.detach().cpu(), os.path.join(matrices_dir, 'final_rho_color.pt'))
    torch.save(model.rho_text.detach().cpu(), os.path.join(matrices_dir, 'final_rho_text.pt'))
    torch.save(model.lambda_color.detach().cpu(), os.path.join(matrices_dir, 'final_lambda_color.pt'))
    torch.save(model.lambda_text.detach().cpu(), os.path.join(matrices_dir, 'final_lambda_text.pt'))
